...
import com.seunghwan.portfolio.domain.repository.*
...
class DataInitializer(
    ...
    fun initializeData() {
        ...
        val introductions = mutableListOf<Introduction>(
            Introduction(content = "안녕", isActive = true),
            Introduction(content = "안녕하", isActive = true),
            Introduction(content = "안녕세요", isActive = true)
        )
        introductionRepository.saveAll(introductions)

        val links = mutableListOf<Link>(
            Link(name = "Github", content = "https://github.com/seungwan", isActive = true),
            Link(name = "Linkedin", content = "http://linkedin.com", isActive = true),
        )
        linkRepository.saveAll(links)

        val experience1 = Experience(
            title = "홍익대학교 세종캠퍼스 (Hongik Univ. Sejong)",
            description = "소프트웨어융합전공",
            startYear = 2022,
            startMonth = 3,
            endYear = 2027,
            endMonth = 12,
        ...
        experience1.addDetails(
            mutableListOf(
                ExperienceDetail(content = "to-put", isActive = true),
                ExperienceDetail(content = "홍익대학교 재학", isActive = true)
            )
        )
        ...
        val django = Skill(name = "DJ", type = SkillType.LANGUAGE.name, isActive = true)
        ...
        project1.addDetails(mutableListOf(
            ProjectDetail(content = "구글 맵스를 활용환 유기묘 발견 지역 정보 제공 API 개발", url = null, isActive = true),
